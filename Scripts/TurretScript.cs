...
public class TurretScript : MonoBehaviour {
	...
	private void RotateTurret()
	{
		...
		if ((Turret_Head.transform.rotation.eulerAngles.y >= 90 && Turret_Head.transform.rotation.eulerAngles.y <= 270)
			|| (Turret_Head.transform.rotation.eulerAngles.y <= 270 && Turret_Head.transform.rotation.eulerAngles.y >= 90)) // determines the range of motion each turret has.
		{
			rotValue *= -1;
		}
	...
	private void FireWeapon()
	{
		if (coolDown <= 0)
		{
			...
			randTime_CoolDown = Random.Range(5.0f,15.0f);
